...
static NannersCodecResult ValidPutInput(const uint8_t* payload, const uint8_t* len, uint8_t bytes){
  if(payload == NULL){
    return NANNERS_CODEC_ERR_PAYLOAD_NULL ;
  }

  if(len == NULL){
    return NANNERS_CODEC_ERR_LENGTH_NULL;
  }
...
NannersCodecResult NannersPutBoolean(uint8_t* payload, uint8_t* len, bool value){
  ...
  const NannersCodecResult result = ValidPutInput(payload, len, bytes);
...
NannersCodecResult NannersPutF32BE(uint8_t* payload, uint8_t* len, float value){
  if(sizeof(float) != 4u){ //Reqires 32-bit float. Should not be an issue on "todays" devices.
    return NANNERS_CODEC_ERR_UNSUPPORTED_TYPE;
  }
